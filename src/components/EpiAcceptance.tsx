...
import { HardHat, Check, ArrowLeft, Package, Calendar, User, FileText, Shield, Pencil, Loader2 } from "lucide-react";
...
export default function EpiAcceptance({ cpf, employeeName, onClose, onAccepted }: EpiAcceptanceProps) {
  ...
  const resetSelection = () => {
    setSelectedDelivery(null); setTermoRead(false);
  };
  ...
  const handleDrawn = async (blob: Blob) => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from("epi-signatures").upload(fileName, blob, { contentType: "image/png" });
      if (uploadError) throw uploadError;
      const { error } = await supabase.rpc("accept_epi_delivery", {
        p_cpf: cpf,
      ...
      <div className="min-h-screen flex flex-col items-center px-4 py-6 relative overflow-auto" style={{ background: pageBg }}>
        <div className="w-full max-w-md">
          ...
          <div className="rounded-2xl p-4 mb-4 border border-white/10" style={{ background: cardBg }}>
            <p className="text-[10px] font-semibold uppercase tracking-wider mb-3" style={{ color: "hsl(210 40% 55%)" }}>Dados do EPI</p>
            <div className="space-y-2.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: "hsl(210 40% 25%)", color: "hsl(210 40% 70%)" }}>{selectedDelivery.epi_category}</span>
              </div>
          ...
          <div className="rounded-2xl p-4 mb-4 border border-white/10" style={{ background: cardBg }}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: "hsl(40 80% 60%)" }}>Termo de Responsabilidade</p>
            </div>
          ...
          {!termoRead ? (
            <label className="flex items-start gap-2 cursor-pointer rounded-xl p-3 border border-white/10 mb-3" style={{ background: "hsl(210 30% 13%)" }}>
              ...
              <span className="text-xs" style={{ color: textMuted }}>Li e compreendi o Termo de Responsabilidade acima e confirmo o recebimento deste EPI em boas condições de uso.</span>
            </label>
          ) : (
      ...
      <div className="w-full max-w-md">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {pending.map(epi => (
              <button key={epi.delivery_id} onClick={() => { setSelectedDelivery(epi); setTermoRead(false); }}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] px-2 py-1 rounded-full font-medium"
                    style={{ background: "hsl(40 80% 20%)", color: "hsl(40 90% 65%)" }}>Pendente</span>
                </div>
              </button>
            ))}
          </div>
